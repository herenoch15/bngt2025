...
//import {DeviceEventEmitter} from '@hkpuits/react-native-beacons-manager'
...
{SET_LIST_POI,
   ...
   SET_POPUP_BT } from '../../store/types/PoiType'
const parseManufacturerData = (data: string | null) => {
  if (!data) return null;
  try {
    ...
    console.error('Error parsing manufacturerData:', err);
    return null;
  }
};

const isIBeacon = (hex: string | null) => {
  return hex?.startsWith('4C000215'); // Apple iBeacon prefix
};
const Accueil = props => {
    ...
    var listPoi = useSelector((state) => state.Poi.listPoi);
  ...
  const startScan = () => {
    ...
    Alert.alert('Scan', 'Started BLE scan...');
    ...
    bleManager.startDeviceScan(
      ...
      (error, device) => {
        ...
        if (device) {
          const parsed = parseManufacturerData(device.manufacturerData);
          ...
          const info = {
            ...
            rssi: device.rssi,
            manufacturerData: parsed || '—',
          };

          if (isIBeacon(parsed)) {
            console.log('📡 📍 iBeacon detected:', info);
          } else {
            console.log('📡 Other BLE device:', info);
          }
        }
      }
    );
    ...
    useEffect(() => {
        ...
        console.log('DEBUT GET BEACONS::::0000')
    ...
    const getBeacons = async  (beaconsList, lang) => {
        console.log('DEBUT GET BEACONS::::DEBUT 05.1025')
      ...
      // Beacons.requestAlwaysAuthorization();
       //Beacons.shouldDropEmptyRanges(true);
        ...
        // Start detecting all iBeacons in the nearby
         console.log('DEBUT GET BEACONS::::DEBUT 05.1')
       console.log('DEBUT 111 Beacons::::::')
       await requestLocationPermission()
       console.log('DEBUT Beacons:::::: 11')
    /*   Beacons.init(); // to set the NotificationChannel, and enable background scanning
       await Beacons.detectIBeacons();
          //   Beacons.startRangingBeaconsInRegion()
       // await Beacons.detectAltBeacons();
      //    await Beacons.Bea();
     await   Beacons.setBackgroundScanPeriod(2000)
      await  Beacons.setForegroundScanPeriod(2000)
        }
        catch(error){}
        console.log('DEBUT Beacons:::::: 222')
        try {
            //   await Beacons.startRangingBeaconsInRegion('region1')
            //   EBEFD083-70A2-47C8-9837-E7B5634DF524
             //   Beacons.startRangingBeaconsInRegion(region) // region.identifier, region.uuid)
            //    Beacons.startMonitoringForRegion(region)
          
          const region=   {
    identifier: 'REGION1',
    uuid: 'EBEFD083-70A2-47C8-9837-E7B5634DF524' // ton UUID beacon
  }
            await Beacons.startMonitoringForRegion(region)
            await Beacons.startRangingBeaconsInRegion(region)
            .then(() => console.log('Beacons ranging started'))
    .catch(error => console.log('Beacons ranging not started', error));
       ...
       */ 
        startRadar('EBEFD083-70A2-47C8-9837-E7B5634DF524', {
  useForegroundService: true,
  useBackgroundScanning: true,
});
        try{
            console.log('DEBUT Beacons:::::: 999999')
           // const { BeaconManager } = NativeModules; // ton module natif
           //const beaconEmitter = new NativeEventEmitter(BeaconManager);
            DeviceEventEmitter.addListener('onBeaconsDetected', async (data) => {
            console.log(' ////// DEBUT beacons // '+ JSON.stringify(data)+ 'NBR::'+JSON.stringify(listPoi))
            var poiTab = []
